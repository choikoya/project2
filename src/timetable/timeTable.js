...
const TimeTable = () => {
  ...
  const fetchData = async () => {
    ...
    socket.onmessage = (event) => {
      ...
      // 처음 로드 시 오늘 날짜 데이터만 필터링
      if (isFirstLoad) {
        const today = new Date().toISOString().slice(0, 10); // 오늘 날짜 yyyy-MM-dd 형식으로 변환
        const todayData = sortedMergedImages.filter(row => {
          const rowDate = parseDateFromName(row.weighbridgename || row.junkyardname);
          return rowDate && rowDate.toISOString().slice(0, 10) === today; // 오늘 날짜만 필터링
        });
        setFilteredData(todayData); // 오늘 날짜 데이터만 표시
        setIsFirstLoad(false); // 처음 로드 완료 상태로 변경
      } else {
        setFilteredData(sortedMergedImages); // 처음 로드 이후에는 전체 데이터 사용
      }
  ...
  const handleSearch = () => {
    setShowAll(false); // "전체" 버튼을 눌러 전체 데이터 보기를 비활성화
